    ...
    .then((data) => {
      if (data.status === "success") {
        ...
        Swal.fire({
          ...
          text:
            data.message || "Un error ocurrio al intentar agregar el producto!",
        });
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      if (data.status === 200) {
        Swal.fire({
          icon: "success",
          title: "Purchase done!",
          text: "You have made your bought!",
        }).then(() => {
          window.location.reload();
        });
      } else {
        Swal.fire({
          icon: "error",
          title: "An error ocurred while buying!!",
          text:
            data.message ||
            "An error ocurred while trying to buy some products!",
        });
      }
    })
    .catch((error) => {
